...
class EventosController extends Controller {
    ...
    public function actionguardarevento() {
        ...
        if ($lugarPartida != "" && $horapartida != "") {
            $id = 1;
            $modelDescripcion = new Descripcion();
            $modelDescripcion->lugar_partida = $lugarPartida;
            $modelDescripcion->hora_partida = $horapartida;
            if ($modelDescripcion->save()) {
                echo "ok descripcion";
            }
        }
        $modelEvento = new Eventos();
        if ($id == 1) {
        $modelEvento->id_descripcion_fk = $modelDescripcion->id;    
        } else {
            
        }
        $modelEvento->nombre = $nombre;
        $modelEvento->descripcion = $descripcion;
        $modelEvento->fecha_inicio = $fecha_ini;
        $modelEvento->fecha_fin = $fecha_fin;
        $modelEvento->precio = $precio;        
        if ($modelEvento->save()) {
            echo "ok Evento";
        }
        echo "paso x aca";
